...
void stencil_sopn_p0_sg(PNSystem::Stencil::Context& ctx)
{
	...
	for( int i=0;i<4;++i )
	{
		...
		M_9(0, 0) = (std::pow(fields.sigma_t->eval(domain.voxelToWorld(vd+ctx.getVoxelSpaceOffsetFromGrid2(i))), 2)+
			-(fields.sigma_t->eval(domain.voxelToWorld(vd+ctx.getVoxelSpaceOffsetFromGrid2(i)))*fields.sigma_s->eval(domain.voxelToWorld(vd+ctx.getVoxelSpaceOffsetFromGrid2(i)))*fields.f_p->eval(0, 0, domain.voxelToWorld(vd+ctx.getVoxelSpaceOffsetFromGrid2(i)))));
	...
	for( int i=0;i<4;++i )
	{
		...
		b(0, 0) = (fields.sigma_t->eval(domain.voxelToWorld(vd+ctx.getVoxelSpaceOffsetFromGrid2(i)))*fields.q->eval(0, 0, domain.voxelToWorld(vd+ctx.getVoxelSpaceOffsetFromGrid2(i))));
