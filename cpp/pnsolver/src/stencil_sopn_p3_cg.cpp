...
void stencil_sopn_p3_cg(PNSystem::Stencil::Context& ctx)
{
	...
	//M_9 ---
	Eigen::Matrix<double, 10, 10> M_9_real_staggered[4];
	for( int i=0;i<4;++i )
	{
	...
	Eigen::Matrix<double, 10, 10> M_9_real;
	...
	//M_10 ---
	Eigen::Matrix<double, 10, 10> M_10_real_staggered[4];
	for( int i=0;i<4;++i )
	{
	...
	Eigen::Matrix<double, 10, 10> M_10_real;
	...
	//M_11 ---
	Eigen::Matrix<double, 10, 10> M_11_real_staggered[4];
	for( int i=0;i<4;++i )
	{
	...
	Eigen::Matrix<double, 10, 10> M_11_real;
	...
	//b ---
	Eigen::Matrix<double, 10, 1> b_real_staggered[4];
	for( int i=0;i<4;++i )
	{
		...
		b(5, 0) = ((0.4472135955*((-h_inv[0]*fields.q->eval(1, 1, domain.voxelToWorld(vd+ctx.getGridOffset2(i))))+
			...
			(h_inv[1]*fields.q->eval(3, 3, domain.voxelToWorld(vd+ctx.getGridOffset2(i))))))+
			(fields.sigma_t->eval(domain.voxelToWorld(vd+ctx.getGridOffset2(i)))*fields.q->eval(2, 2, domain.voxelToWorld(vd+ctx.getGridOffset2(i)))));
		b(6, 0) = (-(0.462910049886*((-h_inv[0]*fields.q->eval(2, -2, domain.voxelToWorld(vd+ctx.getGridOffset2(i))))+
			(h_inv[0]*fields.q->eval(2, -2, domain.voxelToWorld(vd+ctx.getGridOffset2(i))))))+
	...
	Eigen::Matrix<double, 10, 1> b_real;
...
V3i stencil_sopn_p3_cg_get_offset(int coeff)
{
	switch(coeff)
	{
...
REGISTER_STENCIL(stencil_sopn_p3_cg, 3, 10, 2)
