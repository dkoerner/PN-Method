...
namespace cumath
{
	...
	struct Vec3
	{
        ...
        HOST_DEVICE Vec3( );
        HOST_DEVICE Vec3( const T &x, const T &y, const T &z );
		HOST_DEVICE Vec3( const T &xyz );
		template <typename S> HOST_DEVICE Vec3(const Vec3<S> &v);
		HOST_DEVICE ~Vec3( );
		...
		*/
		...
		HOST_DEVICE Vec3<T>                            normalized( void ) const; ///< returnes normalized version if this the vector
		/*
		...
		bool                       operator!=( const Vec3<T> &rhs );
		*/
		HOST_DEVICE bool           operator+=( const Vec3<T> &rhs );
		/*
		bool                       operator-=( const Vec3<T> &rhs );
	...
	template<typename T>
	HOST_DEVICE Vec3<T>::Vec3()
	{
        x=(T)0.0; y=(T)0.0; z=(T)0.0;
    }

	template<typename T>
	HOST_DEVICE Vec3<T>::Vec3( const T &x, const T &y, const T &z )
    {
        this->x=x; this->y=y; this->z=z;
    }

	template<typename T>
	HOST_DEVICE Vec3<T>::Vec3( const T &xyz ) : x(xyz), y(xyz), z(xyz)
	{
	}
	...
	template <typename S>
	inline
	HOST_DEVICE Vec3<T>::Vec3 (const Vec3<S> &v)
	{
		x = T (v.x);
		y = T (v.y);
		z = T (v.z);
	...
	*/
	template<typename T>
	HOST_DEVICE T Vec3<T>::getLength( void ) const
	{
	...
	*/
	template<typename T>
	HOST_DEVICE void Vec3<T>::normalize( void )
	{
		T length = getLength();
	...
	template<typename T>
	HOST_DEVICE Vec3<T> Vec3<T>::normalized( void ) const
	{
		T length = getLength();
	...
	*/
	...
	template<typename T>
	HOST_DEVICE const T& Vec3<T>::operator[]( int i ) const
	{
		return v[i];
	}

	template<typename T>
	HOST_DEVICE T& Vec3<T>::operator[]( int i )
	{
		return v[i];
	}
