...
namespace cumath
{
	...
	struct BoundingBox3
	{
		...
		BoundingBox3( Vec3<T> _minPoint, Vec3<T> _maxPoint );                      ///< constructor
		/*
		BoundingBox3( T minx, T miny, T minz, T maxx, T maxy, T maxz );            ///< constructor
		template<typename S>
		BoundingBox3( const BoundingBox3<S> &other );
		*/
		Vec3<T>                                                size( void ) const;  ///< returns a vector which represents the dimension in each axis
		/*
		...
		bool                                                      isEmpty() const;
		*/
		HOST_DEVICE void                                              makeEmpty();
		//int                                                      maxExtend()const;


		Vec3<T>                                                    minPoint;  ///< the position of all lowends for each axis
		Vec3<T>                                                    maxPoint;  ///< the position of all highends for each axis
	};
	...
	// constructor
	template<typename T>
	BoundingBox3<T>::BoundingBox3( Vec3<T> _minPoint, Vec3<T> _maxPoint )
	{
		minPoint = _minPoint;
		maxPoint = _maxPoint;
	}
	...
	*/
	...
	template<typename T>
	Vec3<T> BoundingBox3<T>::size( void ) const
	{
		return Vec3<T>( maxPoint - minPoint );
	}
	...
	*/
	...
	{
		minPoint = Vec3<T>( (T)std::numeric_limits<T>::max(), (T)std::numeric_limits<T>::max(), (T)std::numeric_limits<T>::max() );
		maxPoint = Vec3<T>( (T)-std::numeric_limits<T>::max(), (T)-std::numeric_limits<T>::max(), (T)-std::numeric_limits<T>::max() );
	}
