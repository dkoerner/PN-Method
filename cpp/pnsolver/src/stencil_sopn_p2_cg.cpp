...
void stencil_sopn_p2_cg(PNSystem::Stencil::Context& ctx)
{
	...
	//M_9 ---
	Eigen::Matrix<double, 6, 6> M_9_real_staggered[4];
	for( int i=0;i<4;++i )
	{
	...
	Eigen::Matrix<double, 6, 6> M_9_real;
	...
	//M_10 ---
	Eigen::Matrix<double, 6, 6> M_10_real_staggered[4];
	for( int i=0;i<4;++i )
	{
	...
	Eigen::Matrix<double, 6, 6> M_10_real;
	...
	//M_11 ---
	Eigen::Matrix<double, 6, 6> M_11_real_staggered[4];
	for( int i=0;i<4;++i )
	{
	...
	Eigen::Matrix<double, 6, 6> M_11_real;
	...
	for( int i=0;i<4;++i )
	{
		...
		b(2, 0) = ((0.408248290464*((-h_inv[0]*fields.q->eval(0, 0, domain.voxelToWorld(vd+ctx.getGridOffset2(i))))+
			...
			(h_inv[1]*fields.q->eval(2, 2, domain.voxelToWorld(vd+ctx.getGridOffset2(i))))))+
			(fields.sigma_t->eval(domain.voxelToWorld(vd+ctx.getGridOffset2(i)))*fields.q->eval(1, 1, domain.voxelToWorld(vd+ctx.getGridOffset2(i)))));
		b(3, 0) = (-(0.4472135955*((-h_inv[0]*fields.q->eval(1, -1, domain.voxelToWorld(vd+ctx.getGridOffset2(i))))+
			(h_inv[0]*fields.q->eval(1, -1, domain.voxelToWorld(vd+ctx.getGridOffset2(i))))))+
...
V3i stencil_sopn_p2_cg_get_offset(int coeff)
{
	switch(coeff)
	{
...
REGISTER_STENCIL(stencil_sopn_p2_cg, 2, 6, 2)
