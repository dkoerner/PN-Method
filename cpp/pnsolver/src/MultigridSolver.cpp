...
MultigridSolver::Level* MultigridSolver::addLevel()
{
	int index = m_levels.size();
	Level* lvl = new Level(index);

	if(!m_levels.empty())
		m_levels.back()->coarse = lvl;

	m_levels.push_back(lvl);
	return lvl;
}

MultigridSolver::Level* MultigridSolver::getLevel(int index)
{
	return m_levels[index];
}

int MultigridSolver::getNumLevels()const
{
