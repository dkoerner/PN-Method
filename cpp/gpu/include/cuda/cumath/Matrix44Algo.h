...
namespace cumath
{
	...
	*/
	...
	template<typename T>
	HOST_DEVICE inline Vec3<T> transform(  const Vec3<T> &lhs, const Matrix44<T> &rhs )
	{
	...
	template<typename T>
	HOST_DEVICE inline void transform( Vec3<T> &result, const Vec3<T> &lhs, const Matrix44<T> &rhs )
	{
	...
	// same as transform
	template<typename T>
	HOST_DEVICE inline Vec3<T> operator*( const Vec3<T> &lhs, const Matrix44<T> &rhs )
	{
		return Vec3<T>(lhs.x*rhs._11 + lhs.y*rhs._21 + lhs.z*rhs._31 + rhs._41,
					 lhs.x*rhs._12 + lhs.y*rhs._22 + lhs.z*rhs._32 + rhs._42,
					 lhs.x*rhs._13 + lhs.y*rhs._23 + lhs.z*rhs._33 + rhs._43 );
	...
	/*
	template<typename T>
	inline Matrix44<T> &transpose( Matrix44<T> &matrix )
	{
		matrix.transpose();
		return matrix;
	}
