...
namespace moexp
{
	...
	double P_real(int l, int m, double x)
	{
		// Compute Pmm(x) = (-1)^m(2m - 1)!!(1 - x^2)^(m/2), where !! is the double
		// factorial.
		double pmm = 1.0;
...
void rasterizeSphericalFunctionMap( const std::string& filename, moexp::SphericalFunction<Color3f> func, const V2i& resolution )
{
	EnvMap envmap(resolution);
	//std::ofstream f( "test_pixels_coords.txt", std::ios::binary | std::ios::trunc );
	int xres = envmap.bitmap().cols();
	int yres = envmap.bitmap().rows();
	for( int j=0;j<yres;++j )
...
void rasterizeSphericalFunctionMap2( EnvMap& envmap, moexp::SphericalFunction<Color3f> func )
{
	//std::ofstream f( "test_pixels_coords.txt", std::ios::binary | std::ios::trunc );
	int xres = envmap.bitmap().cols();
	int yres = envmap.bitmap().rows();
