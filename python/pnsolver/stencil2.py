...
def generate_stencil_code( pne, pni, stencil_name ):
	...
	cpp += "// This file was generated by {}\n\n".format(__file__)
	...
	for coeff_index in range(numCoeffs):
		...
		for op in ops:
			...
			if root.__class__ == Coefficient:
				...
				# we expect an unknown as part of the product
				...
				#unknown_id = unknown.getSymbol() + "[{}]".format(index, i, j, k)
		...
		for unknown_id, coeffs in unknown_coeffs.items():
			...
			#code = generate_cpp_recursive(coeffs.getExpr())
			#cpp += "\tctx.coeff_A( {}, vi+V3i({}, {}, {}), {} ) += {};\n".format(coeff_index, i, j, k, coeff_index_j, code)

			## target coefficient
			#if unknown is None:
			#	# RHS
			#	print( "\tctx.coeff_b( {} ) += {};".format(coeff_index, code) )
			#else:
			#	# LHS
