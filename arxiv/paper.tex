...
Since the real-valued SH bases (Equation~\ref{eq:sh_real_basis}) have different definitions for $m<0$, $m=0$ or $m>0$,  we get different projections for $\mathcal{S}^{l,m}$, depending on the sign of $m$. (Note that in cases where $L^{l,m}$ would appear with $l<0$ or $|m|>l$, this term is implicitly set to zero.)
%\vspace{5in}
%\begin{align}
...
\begin{eqnarray}
&&\frac{1}{\sqrt{2}} \left( 
         c^{\scaleto{l-1,-1}{4pt}} \partial_x L^{\scaleto{l-1,1}{4pt}}
        -d^{\scaleto{l+1,-1}{4pt}} \partial_x L^{\scaleto{l+1,1}{4pt}}  \right)
\nonumber \\
&+& \frac{1}{\sqrt{2}} \left(
         c^{\scaleto{l-1,-1}{4pt}} \partial_y L^{\scaleto{l-1,-1}{4pt}}
        -d^{\scaleto{l+1,-1}{4pt}} \partial_y L^{\scaleto{l+1,-1}{4pt}}  \right)
\nonumber \\
...
&+\frac{1}{2} \beta_x^{\scaleto{m}{4pt}} f^{\scaleto{l+1,\pm m+1}{4pt}} \partial_x L^{\scaleto{l+1,m\pm 1}{4pt}}
\nonumber \\
...
\nonumber \\
 \mp \frac{1}{2} \beta_y^{\scaleto{m}{4pt}} e^{\scaleto{l-1,\pm m+1}{4pt}} \partial_y L^{\scaleto{l-1,-m\mp 1}{4pt}}
&\pm \frac{1}{2} \beta_y^{\scaleto{m}{4pt}} f^{\scaleto{l+1,\pm m+1}{4pt}} \partial_y L^{\scaleto{l+1,-m\mp 1}{4pt}}
\nonumber \\
...
%\vspace{-0.2in}
\icaption{\emph{Staggering} distributes the coefficients of the solution vector $\vec{u}$ onto disjoint grids, indicated by the symbols, in a way which guarantees second order accuracy~\cite{Seibold14}.\vspace{-.2in}}
...
\tocless\subsection{Runtime \label{sec:solver_runtime}}
...
\right.
\end{align}
Note we use the subscript $\mathbb{R}$ and $\mathbb{C}$ to distinguish between real- and complex-valued SH basis functions respectively.
