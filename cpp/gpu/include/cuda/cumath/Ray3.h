...
namespace cumath
{
	...
	public:
		...
		HOST_DEVICE Ray3();                                                                                   // constructor
		HOST_DEVICE Ray3( const cumath::Vec3<T> &origin, const cumath::Vec3<T> &direction, const T &_tmin, const T &_tmax);       // constructor


		HOST_DEVICE Vec3<T>                                              getPosition( T t )const; // returns origin+direction*t

		Vec3<T>                                                                                o; // point in space where the ray originates from
		Vec3<T>                                                                                d; // normalized direction of the ray
	...
	// constructor
	template<typename T>
	HOST_DEVICE Ray3<T>::Ray3() : tmin(0.0f), tmax(FLT_MAX)
	{
	}

	// constructor
	template<typename T>
	HOST_DEVICE Ray3<T>::Ray3( const cumath::Vec3<T> &origin, const cumath::Vec3<T> &direction, const T &_tmin, const T &_tmax ) : o(origin), d(direction), tmin(_tmin), tmax(_tmax)
	{
	}

	// returns origin+direction*t
	template<typename T>
	HOST_DEVICE Vec3<T> Ray3<T>::getPosition( T t )const
	{
		return o + d*t;
	}
	...
	*/
	template<typename T>
	inline HOST_DEVICE bool intersectionRayBox( const Ray3<T> &ray, const BoundingBox3<T> &box, T &hitt0, T &hitt1, bool debug = false )
	{
		/*
		//Vec3<T> dL(1.0f/ray.d.x, 1.0f/ray.d.y, 1.0f/ray.d.z);
		Vec3<T> dL(1.0f, 1.0f, 1.0f);
		if( ray.d.x != T(0.0) )
			dL.x /= ray.d.x;
		if( ray.d.y != T(0.0) )
			dL.y /= ray.d.y;
		if( ray.d.z != T(0.0) )
			dL.z /= ray.d.z;
		Vec3<T> lo = (box.minPoint - ray.o) * dL;
		Vec3<T> hi = (box.maxPoint - ray.o) * dL;
		Vec3<T> tmp=min(lo,hi); 
		hi=lo+hi-tmp; 
		lo=tmp;
		bool hit = !( lo.x>hi.y || lo.y>hi.x || lo.x>hi.z || lo.z>hi.x || lo.y>hi.z || lo.z>hi.y );
		hitt0 = max(max(max( ray.tmin, lo.x), lo.y), lo.z);
		hitt1 = min(min(min( ray.tmax, hi.x), hi.y), hi.z);
		return hit;
		*/

		///*
		T t0 = ray.tmin, t1 = ray.tmax;
		//T t0 = MATH_NEG_INFTY_F;
		//T t1 = CUDART_INF_F;
		for (int i = 0; i < 3; ++i)
		{
			// Update interval for _i_th bounding box slab
			T invRayDir = (T)1.0 / ray.d[i];
			T tNear = (box.minPoint[i] - ray.o[i]) * invRayDir;
			T tFar  = (box.maxPoint[i] - ray.o[i]) * invRayDir;
